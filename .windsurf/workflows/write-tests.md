---
description: Write or update dollar-shell tests for a module or feature
---
...
1. Read the existing test files in `tests/` to understand patterns and conventions used in this project.
2. Identify the module or feature to test. Read its source code to understand the public API.
3. Create or update the test file in `tests/test-<name>.js`:
...
5. Run the full test suite to check for regressions: `npm test`
   - If debugging, run a single file: `node tests/test-<name>.js`
6. Run TypeScript checking: `npm run ts-check`
