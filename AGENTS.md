...
- `src/index.js` is the main entry point. At import time it selects the runtime backend, then calls `buildApi()` (`src/build.js`), which picks the platform shell module and wires up the tag functions. Setting `globalThis.DSH_FORCE_NODE` or the `DSH_FORCE_NODE` env var forces the Node backend on every runtime (Bun/Deno via their `node:child_process` compat).
- `dollar-shell/node` (`src/node/index.js`) is a second entry: the identical API built by the same `buildApi`, but on the Node backend's raw-streams variant, so `stdin`/`stdout`/`stderr` are Node streams (and `asDuplex`/`.io`/`.through` a Node `Duplex`) instead of web streams.
